...
-- Handle players who are already in the game when the script starts
for _, player in ipairs(Players:GetPlayers()) do
    player.Chatted:Connect(function(message)
        onChatted(player, message)
    end)

    -- Handle character loading for existing players (important!)
    if player.Character then
        -- If the character exists, connect the CharacterRemoving event
        player.CharacterRemoving:Connect(function(character)
             if loopKillPlayers[player.Name] then  --check before unlooping
                unloopKill(player) -- Clean up loopkill if character dies
            end
            removeBlackHole(player); --clean up blackhole
        end)
    else
        -- If character doesn't exist yet, wait for it to be added
...
-- Handle players who join the game *after* the script starts
